...
@Component({
  selector: 'app-questionnaire-enseignant',
  templateUrl: './questionnaire-enseignant.component.html',
  styleUrls: ['./questionnaire-enseignant.component.scss']
})
export class QuestionnaireEnseignantComponent implements OnInit {
  CodeNiveau:string;
...
getInfo(Code_ecole,Code_Matiere,Code_Niveau){
  ...
  .subscribe(
    info_Ecole =>{ 
      ...
      this.currentItemNumber=1;
  //console.log(Code_el_eva);
      this.getAllItems(this.CodeEcole.Code_ecole,'Qe',this.CodeNiveau_Temp);
...
getAllItems(CodeEleve:String,CodeMatiere:String,CodeNiveau){
   CodeEleve = CodeEleve.endsWith('e')? CodeEleve:CodeEleve+'e';
...
this.info ={"CodeEleve":this.CodeEcole.Code_ecole+'e',CodeItem:CodeItem_,"valeur":val,"OrdreItem":OrdreItem_};
