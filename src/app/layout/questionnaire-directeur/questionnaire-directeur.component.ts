import { Infecole } from './../../shared/model/infecole';
import { Ecole } from './../../shared/model/ecole';
...
@Component({
  selector: 'app-questionnaire-directeur',
  templateUrl: './questionnaire-directeur.component.html',
  styleUrls: ['./questionnaire-directeur.component.scss']
})
export class QuestionnaireDirecteurComponent implements OnInit {
  CodeNiveau:string;
...
//currentItem:SaiListItem;
errorMessage:string;
title:string="List des Items Ecoles";
items:SaiItem[];//Item[] =[];
itemsTempon:SaiItem[];
nbItem:number=0;
currentItemNumber:number =1;
constructor(private _reponseService:ReponseService,private itemsService:ItemsService,private _matiereService:MatiereService,private _ecoleServiceService:EcoleServiceService) { }
obj_ :SaiItem = null;
ngOnInit() {
...
prochainItem(){
 ...
 filterIt(){
   this.initCurrentItem();
    this.getAllItems(this.CodeEcole.Code_ecole,this.CodeMatiere_Temp,this.CodeNiveau_Temp);
 }
...
getInfo(Code_ecole,Code_Matiere,Code_Niveau){
  this.CodeMatiere_Temp = Code_Matiere;
  this.CodeNiveau_Temp = Code_Niveau;
  //console.log(Code_el_eva);
  this.CodeEcole={'Code_ecole':Code_ecole};
  console.log(this.CodeEcole.Code_ecole ,this.CodeMatiere_Temp);
  // = {'Code_el_eva':Code_el_eva};
  this._ecoleServiceService.getInfoEcole(this.CodeEcole)
  .subscribe(
    info_Ecole =>{ 
      //this.infoEleve = <InfoEleve>info_Ecole;
      this.infoEcole = <Infecole>info_Ecole;
      this.currentItemNumber=1;
  //console.log(Code_el_eva);
      this.getAllItems(this.CodeEcole.Code_ecole,'Qd',this.CodeNiveau_Temp);
      console.log(this.infoEcole );
      console.log("----------" );
      console.log(info_Ecole );
...
getAllItems(CodeEleve:String,CodeMatiere:String,CodeNiveau){
   CodeEleve = CodeEleve.endsWith('d')? CodeEleve:CodeEleve+'d';
   console.log('Code Ecole :',CodeEleve);
...
this.info ={"CodeEleve":this.CodeEcole.Code_ecole+'d',CodeItem:CodeItem_,"valeur":val,"OrdreItem":OrdreItem_};
this._reponseService.UpdateReponse(this.info).subscribe(p=>{
  this.n = this.currentItemNumber;
  console.log('koli',this.info);
