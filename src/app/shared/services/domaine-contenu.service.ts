...
import { DomaineContenu } from '../model/domaine-contenu';
...
@Injectable()
export class DomaineContenuService{
  constructor(private _http:Http,private _connexionService:ConnexionService){}
  domaineId:number =100;
  getAllDomaine():Observable<DomaineContenu[]>{
    return this._http.get(this._connexionService.chemin+'/eval/api/domaine/listDomaineContenu.php')
                 .map(response => <DomaineContenu[]>response.json())
                 .do(data =>{
